...
export interface MediaTitlesItem {
  name: string;
  id: number;
}

// TODO: replace this with real data from your application
const EXAMPLE_DATA: MediaTitlesItem[] = [
  {id: 1, name: 'Hydrogen'},
  {id: 2, name: 'Helium'},
  {id: 3, name: 'Lithium'},
  {id: 4, name: 'Beryllium'},
...
export class MediaTitlesDataSource extends DataSource<MediaTitle> {
  ...
  loadTitles(page = 1, pageSize = 5) {
    ...
    this.titlesService.getMediaTitles(page, pageSize).pipe(
      ...
      d => {
        const dataPage = d as DataPage<MediaTitle>;
        this.data.next(dataPage.items);
        this.count = dataPage.count;
      }
    );
  }
   ...
   */
  private getPagedData(data: MediaTitlesItem[]) {
    const startIndex = this.paginator.pageIndex * this.paginator.pageSize;
    return this.loadTitles(this.paginator.pageIndex, this.paginator.pageSize);
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: MediaTitlesItem[]) {
